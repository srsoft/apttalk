  ...
  <v-card>
    ...
    <v-card-actions>
      ...
      <span class="font-italic caption">
        작성일 : <DisplayTime :time="item.createdAt"></DisplayTime>
      </span>
    </v-card-actions>
    <v-card-actions>
      <v-spacer />
      <span class="font-italic caption">
        수정일 : <DisplayTime :time="item.updatedAt"></DisplayTime>
      </span>
    </v-card-actions>
    <v-divider></v-divider>
    <DisplayComment :docRef="this.ref.collection('articles').doc(this.item.id)" />
  </v-card>
</template>
