  ...
  <div>
    ...
    <v-list nav>
      ...
      >
        ...
        >
          <v-list-item-content>
            <v-list-item-title :class="$store.state.editable ? 'subItemPL' : ''">
              {{ subItem.title }}
              <span v-if="$store.state.editable">
                <v-btn icon @click="openDialogSubItem(i, j)"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn icon @click="moveItem(item.subItems, j, -1)" v-if="i > 0"><v-icon>mdi-chevron-double-up</v-icon></v-btn>
                <v-btn icon @click="moveItem(item.subItems, j, 1)" v-if="i < item.subItems.length - 1"><v-icon>mdi-chevron-double-down</v-icon></v-btn>
        ...
        <v-list-item @click="openDialogSubItem(i, -1)" v-if="$store.state.editable">
          <v-list-item-icon :class="$store.state.editable ? 'subItemPL' : ''">
